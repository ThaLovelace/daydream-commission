...
export default function EntryCard({ entry, index, onClick, onDelete, showDate }) {
  const branchClass = BRANCH_STYLE[entry.branch?.name] || 'bg-primary-soft text-primary-dark';

  return (
    <div
      onClick={() => onClick && onClick(entry)}
      className="rounded-2xl border border-line bg-white p-3.5 flex items-start justify-between gap-3 active:bg-primary-soft/40 cursor-pointer"
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center flex-wrap gap-x-2 gap-y-1 text-xs text-ink-faint mb-1.5">
          {typeof index === 'number' && (
            <span className="font-semibold text-ink-soft">ลูกค้าคนที่ {index + 1}</span>
          )}
